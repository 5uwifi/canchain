...
func (pool *TxPool) SubscribeNewTxsEvent(ch chan<- kernel.NewTxsEvent) event.Subscription {
...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	if from, err = types.Sender(pool.signer, tx); err != nil {
		return kernel.ErrInvalidSender
	}
	currentState := pool.currentState(ctx)
	if n := currentState.GetNonce(from); n > tx.Nonce() {
		return kernel.ErrNonceTooLow
	}

	header := pool.chain.GetHeaderByHash(pool.head)
	if header.GasLimit < tx.Gas() {
		return kernel.ErrGasLimit
	}

	if tx.Value().Sign() < 0 {
		return kernel.ErrNegativeValue
	}

	if b := currentState.GetBalance(from); b.Cmp(tx.Cost()) < 0 {
		return kernel.ErrInsufficientFunds
	}

	gas, err := kernel.IntrinsicGas(tx.Data(), tx.To() == nil, pool.homestead)
	if err != nil {
		return err
	}
	if tx.Gas() < gas {
		return kernel.ErrIntrinsicGas
	}
...
func (self *TxPool) add(ctx context.Context, tx *types.Transaction) error {
	...
	if _, ok := self.pending[hash]; !ok {
		...
		go self.txFeed.Send(kernel.NewTxsEvent{Txs: types.Transactions{tx}})
	}

	log4j.Debug("Pooled new transaction", "hash", hash, "from", log4j.Lazy{Fn: func() common.Address { from, _ := types.Sender(self.signer, tx); return from }}, "to", tx.To())
	return nil
}
