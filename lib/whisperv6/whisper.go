...
func (whisper *Whisper) MinPow() float64 {
	...
	v, ok := val.(float64)
	if !ok {
		log4j.Error("Error loading minPowIdx, using default")
		return DefaultMinimumPoW
	}
	return v
}
...
func (whisper *Whisper) notifyPeersAboutPowRequirementChange(pow float64) {
	...
	for _, p := range arr {
		...
		if err != nil {
			log4j.Warn("failed to notify peer about new pow requirement", "peer", p.ID(), "error", err)
		}
	}
}
...
func (whisper *Whisper) notifyPeersAboutBloomFilterChange(bloom []byte) {
	...
	for _, p := range arr {
		...
		if err != nil {
			log4j.Warn("failed to notify peer about new bloom filter", "peer", p.ID(), "error", err)
		}
	}
}
	...
	for {
		...
		case messagesCode:
			...
			if err := packet.Decode(&envelopes); err != nil {
				log4j.Warn("failed to decode envelopes, peer will be disconnected", "peer", p.peer.ID(), "err", err)
		...
		case powRequirementCode:
			...
			i, err := s.Uint()
			if err != nil {
				log4j.Warn("failed to decode powRequirementCode message, peer will be disconnected", "peer", p.peer.ID(), "err", err)
				return errors.New("invalid powRequirementCode message")
			}
			f := math.Float64frombits(i)
			if math.IsInf(f, 0) || math.IsNaN(f) || f < 0.0 {
				log4j.Warn("invalid value in powRequirementCode message, peer will be disconnected", "peer", p.peer.ID(), "err", err)
		...
		case bloomFilterExCode:
			...
			if err != nil {
				log4j.Warn("failed to decode bloom filter exchange message, peer will be disconnected", "peer", p.peer.ID(), "err", err)
