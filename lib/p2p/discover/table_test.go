...
func testPingReplace(t *testing.T, newNodeIsResponding, lastInBucketIsResponding bool) {
	...
	pingSender := wrapNode(cnode.NewV4(&pingKey.PublicKey, net.IP{}, 99, 99))
...
func TestBucket_bumpNoDuplicates(t *testing.T) {
	...
	cfg := &quick.Config{
		...
		Values: func(args []reflect.Value, rand *rand.Rand) {
			...
			nodes := make([]*node, n)
			for i := range nodes {
				nodes[i] = nodeAtDistance(cnode.ID{}, 200, intIP(200))
			}
	...
	test := func(test *closeTest) bool {
		...
		for _, b := range tab.buckets {
			for _, n := range b.entries {
				...
				if cnode.DistCmp(test.Target, n.ID(), farthestResult) < 0 {
...
func TestTable_ReadRandomNodesGetAll(t *testing.T) {
	cfg := &quick.Config{
		...
		Rand:     rand.New(rand.NewSource(time.Now().Unix())),
		Values: func(args []reflect.Value, rand *rand.Rand) {
			args[0] = reflect.ValueOf(make([]*cnode.Node, rand.Intn(1000)))
		},
	}
	test := func(buf []*cnode.Node) bool {
		transport := newPingRecorder()
		tab, db := newTestTable(transport)
		defer tab.Close()
		defer db.Close()
...
func (*closeTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &closeTest{
		Self:   gen(cnode.ID{}, rand).(cnode.ID),
		Target: gen(cnode.ID{}, rand).(cnode.ID),
		N:      rand.Intn(bucketSize),
	}
	for _, id := range gen([]cnode.ID{}, rand).([]cnode.ID) {
		n := cnode.SignNull(new(enr.Record), id)
...
func TestTable_Lookup(t *testing.T) {
	...
	seed := wrapNode(cnode.NewV4(seedKey, net.IP{}, 0, 256))
	tab.stuff([]*node{seed})

	results := tab.lookup(lookupTestnet.target, true)
	t.Logf("results:")
	for _, e := range results {
		t.Logf("  ld=%d, %x", cnode.LogDist(lookupTestnet.targetSha, e.ID()), e.ID().Bytes())
	}
...
var lookupTestnet = &preminedTestnet{
	...
	targetSha: cnode.HexID("5c944ee51c5ae9f72a95eccb8aed0374eecb5119d720cbea6813e8e0d6ad9261"),
	dists: [257][]encPubkey{
		240: {
...
func (tn *preminedTestnet) self() *cnode.Node {
	return nullNode
}

func (tn *preminedTestnet) findnode(toid cnode.ID, toaddr *net.UDPAddr, target encPubkey) ([]*node, error) {
	if toaddr.Port == 0 {
	...
	for i, ekey := range tn.dists[toaddr.Port] {
		...
		node := wrapNode(cnode.NewV4(key, net.ParseIP("127.0.0.1"), i, next))
...
func (*preminedTestnet) waitping(from cnode.ID) error                  { return nil }
func (*preminedTestnet) ping(toid cnode.ID, toaddr *net.UDPAddr) error { return nil }
...
func (tn *preminedTestnet) mine(target encPubkey) {
	...
	for found < bucketSize*10 {
		...
		ld := cnode.LogDist(tn.targetSha, key.id())
