...
func makeProvers(trie *Trie) []func(key []byte) *candb.MemDatabase {
	var provers []func(key []byte) *candb.MemDatabase

	provers = append(provers, func(key []byte) *candb.MemDatabase {
		proof := candb.NewMemDatabase()
