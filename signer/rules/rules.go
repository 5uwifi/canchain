...
func (r *rulesetUI) checkApproval(jsfunc string, jsarg []byte, err error) (bool, error) {
	...
	if err != nil {
		log4j.Info("error occurred during execution", "error", err)
		return false, err
	}
	result, err := v.ToString()
	if err != nil {
		log4j.Info("error occurred during response unmarshalling", "error", err)
		return false, err
	}
	if result == "Approve" {
		log4j.Info("Op approved")
		return true, nil
	} else if result == "Reject" {
		log4j.Info("Op rejected")
		return false, nil
	}
...
func (r *rulesetUI) ApproveTx(request *core.SignTxRequest) (core.SignTxResponse, error) {
	...
	if err != nil {
		log4j.Info("Rule-based approval error, going to manual", "error", err)
...
func (r *rulesetUI) OnSignerStartup(info core.StartupInfo) {
	...
	if err != nil {
		log4j.Warn("failed marshalling data", "data", info)
		return
	}
...
func (r *rulesetUI) OnApprovedTx(tx canapi.SignTransactionResult) {
	jsonTx, err := json.Marshal(tx)
	if err != nil {
		log4j.Warn("failed marshalling transaction", "tx", tx)
		return
	}
