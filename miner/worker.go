...
type Work struct {
	...
	gasPool   *kernel.GasPool // available gas used to pack transactions
...
func (self *worker) commitNewWork() {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		log4j.Info("Commit new mining work", "number", work.Block.Number(), "txs", work.tcount, "uncles", len(uncles), "elapsed", common.PrettyDuration(time.Since(tstart)))
...
func (env *Work) commitTransactions(mux *event.TypeMux, txs *types.TransactionsByPriceAndNonce, bc *kernel.BlockChain, coinbase common.Address) {
	if env.gasPool == nil {
...
func (env *Work) commitTransaction(tx *types.Transaction, bc *kernel.BlockChain, coinbase common.Address, gp *kernel.GasPool) (error, []*types.Log) {
