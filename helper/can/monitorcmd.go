...
var (
	...
	monitorCommand = cli.Command{
		...
		Description: `
The CAN monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	if client, err = dialRPC(endpoint); err != nil {
		utils.Fatalf("Unable to attach to CAN node: %v", err)
	}
	defer client.Close()
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by CAN (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
