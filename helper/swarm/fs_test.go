...
func TestCLISwarmFs(t *testing.T) {
	...
	log4j.Debug("swarmfs cli test", "1st mount", mountPoint)
	if err != nil {
		t.Fatal(err)
	}
	...
	log4j.Debug("swarmfs cli test: mounting first run", "ipc path", filepath.Join(handlingNode.Dir, handlingNode.IpcPath))
	...
	hashRegexp := `[a-f\d]{64}`
	log4j.Debug("swarmfs cli test: unmounting first run...", "ipc path", filepath.Join(handlingNode.Dir, handlingNode.IpcPath))
	...
	log4j.Debug("swarmfs cli test: asserting no files in mount point")
	...
	log4j.Debug("swarmfs cli test", "2nd mount point at", secondMountPoint)
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(secondMountPoint)

	log4j.Debug("swarmfs cli test: remounting at second mount point", "ipc path", filepath.Join(handlingNode.Dir, handlingNode.IpcPath))
	...
	log4j.Debug("swarmfs cli test: traversing file tree to see it matches previous mount")
	...
	log4j.Debug("swarmfs cli test: unmounting second run", "ipc path", filepath.Join(handlingNode.Dir, handlingNode.IpcPath))
...
func doUploadEmptyDir(t *testing.T, node *testNode) string {
	...
	log4j.Info("swarmfs cli test: uploading dir with 'swarm up'")
	up := runSwarm(t, flags...)
	_, matches := up.ExpectRegexp(hashRegexp)
	up.ExpectExit()
	hash := matches[0]
	log4j.Info("swarmfs cli test: dir uploaded", "hash", hash)
	return hash
}
