...
func testCLISwarmUp(toEncrypt bool, t *testing.T) {
	log4j.Info("starting 3 node cluster")
	...
	log4j.Info(fmt.Sprintf("uploading file with 'swarm up'"))
	up := runSwarm(t, flags...)
	_, matches := up.ExpectRegexp(hashRegexp)
	up.ExpectExit()
	hash := matches[0]
	log4j.Info("file uploaded", "hash", hash)

	// get the file from the HTTP API of each node
	for _, node := range cluster.Nodes {
		log4j.Info("getting file from node", "node", node.Name)
		...
		log4j.Debug("verifying uploaded file using `swarm down`")
...
func testCLISwarmUpRecursive(toEncrypt bool, t *testing.T) {
	...
	up.ExpectExit()
	hash := matches[0]
	log4j.Info("dir uploaded", "hash", hash)
