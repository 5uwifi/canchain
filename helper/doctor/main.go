...
import (
	...
	"github.com/5uwifi/canchain/basis/whisper/mailserver"
...
var (
	...
	argVerbosity = flag.Int("verbosity", int(log4j.LvlError), "log verbosity level")
	...
	argEnode   = flag.String("boot", "", "bootstrap node you want to connect to (e.g. ccnode://e454......08d50@52.176.211.200:16428)")
...
func initialize() {
	log4j.Root().SetHandler(log4j.LvlFilterHandler(log4j.Lvl(*argVerbosity), log4j.StreamHandler(os.Stderr, log4j.TerminalFormat(false))))
	...
	if *bootstrapMode {
		...
		*bootstrapMode = true
		...
		if len(*argEnode) == 0 {
			argEnode = scanLineA("Please enter the peer's ccnode: ")
		}
	...
	if *mailServerMode {
		if len(msPassword) == 0 {
			msPassword, err = repl.Stdin.PromptPassword("Please enter the Mail Server password: ")
			if err != nil {
...
func configureNode() {
	...
	if !*asymmetricMode && !*forwarderMode {
		if len(symPass) == 0 {
			symPass, err = repl.Stdin.PromptPassword("Please enter the password for symmetric encryption: ")
			if err != nil {
...
func extractIDFromEnode(s string) []byte {
	...
	if err != nil {
		utils.Fatalf("Failed to parse ccnode: %s", err)
	}
	return n.ID[:]
}
