...
func (c *Client) reconnect(ctx context.Context) error {
	...
	if err != nil {
		log4j.Trace(fmt.Sprintf("reconnect failed: %v", err))
		return err
	}
	select {
...
func (c *Client) dispatch(conn net.Conn) {
	...
	for {
		...
		case err := <-c.readErr:
			log4j.Debug("<-readErr", "err", err)
			c.closeRequestOps(err)
			conn.Close()
			reading = false

		case newconn := <-c.reconnected:
			log4j.Debug("<-reconnected", "reading", reading, "remote", conn.RemoteAddr())
			if reading {
...
func (c *Client) handleNotification(msg *jsonrpcMessage) {
	if !strings.HasSuffix(msg.Method, notificationMethodSuffix) {
		log4j.Debug("dropping non-subscription message", "msg", msg)
		return
	}
	...
	if err := json.Unmarshal(msg.Params, &subResult); err != nil {
		log4j.Debug("dropping invalid subscription message", "msg", msg)
		return
	}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	if op == nil {
		log4j.Debug("unsolicited response", "msg", msg)
		return
	}
