...
func init() {
	...
	if *vmodule != "" {
		...
		glogger := log4j.NewGlogHandler(log4j.StreamHandler(colorable.NewColorableStderr(), log4j.TerminalFormat(true)))
		if *verbosity > 0 {
...
func runOverlaySim() {
	...
	log4j.Info(fmt.Sprintf("starting simulation server on 0.0.0.0:%d...", httpSimPort))
