...
func testSwarmNetwork(t *testing.T, o *testSwarmNetworkOptions, steps ...testSwarmNetworkStep) {
	...
	services := map[string]adapters.ServiceFunc{
		"swarm": func(ctx *adapters.ServiceContext) (node.Service, error) {
			...
			log4j.Info("new swarm", "bzzKey", config.BzzKey, "baseAddr", fmt.Sprintf("%x", s.bzz.BaseAddr()))
	...
	for i, step := range steps {
		log4j.Debug("test sync step", "n", i+1, "nodes", step.nodeCount)
		...
		for _, id := range nodeIDs {
			...
			log4j.Trace("file uploaded", "node", id, "key", key.String())
		...
		result := sim.Run(ctx, &simulations.Step{
			Action: func(ctx context.Context) error {
				if *waitKademlia {
					...
					for range ticker.C {
						healthy := true
						log4j.Debug("kademlia health check", "node count", len(nIDs), "addr count", len(addrs))
						for i, id := range nIDs {
							swarm := swarms[id]
							...
							//print info
							log4j.Debug(swarm.bzz.String())
							log4j.Debug("kademlia", "empty bins", pp.EmptyBins, "gotNN", h.GotNN, "knowNN", h.KnowNN, "full", h.Full)
							log4j.Debug("kademlia", "health", h.GotNN && h.KnowNN && h.Full, "addr", fmt.Sprintf("%x", swarm.bzz.BaseAddr()), "id", id, "i", i)
							log4j.Debug("kademlia", "ill condition", !h.GotNN || !h.Full, "addr", fmt.Sprintf("%x", swarm.bzz.BaseAddr()), "id", id, "i", i)
							if !h.GotNN || !h.Full {
								healthy = false
								break
							}
		...
		log4j.Debug("done: test sync step", "n", i+1, "nodes", step.nodeCount)
	}
}
...
func addNodes(count int, net *simulations.Network) (ids []discover.NodeID, err error) {
	for i := 0; i < count; i++ {
		...
		log4j.Debug("created node", "id", node.ID())
		...
		if l > 0 {
			...
			log4j.Debug("connect nodes", "one", node.ID(), "other", otherNodeID)
...
func removeNodes(count int, net *simulations.Network) error {
	for i := 0; i < count; i++ {
		...
		log4j.Debug("removed node", "id", node.ID())
	}
	return nil
}
...
func retrieve(
	...
	for _, id := range nodeIDs {
		...
		for _, f := range files {
			...
			go func(f file, id discover.NodeID) {
				...
				log4j.Debug("api get: check file", "node", id.String(), "key", f.addr.String(), "total files found", atomic.LoadUint64(totalFoundCount))
				...
				log4j.Info("api get: file found", "node", id.String(), "key", f.addr.String(), "content", data, "files found", atomic.LoadUint64(&foundCount))
			}(f, id)
		}
		...
		go func(id discover.NodeID) {
			...
			if foundCount == checkCount {
				log4j.Info("all files are found for node", "id", id.String(), "duration", time.Since(start))
				nodeStatusM.Store(id, 0)
				trigger <- id
				return
			}
			log4j.Debug("files missing for node", "id", id.String(), "check", checkCount, "found", foundCount)
		}(id)
	...
	for err := range errc {
		...
		log4j.Warn(err.Error())
	}

	log4j.Info("check stats", "total check count", totalCheckCount, "total files found", atomic.LoadUint64(totalFoundCount), "total errors", errCount)
