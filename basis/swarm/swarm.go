...
import (
	...
	"github.com/5uwifi/canchain/contracts/chequebook"
	...
	httpapi "github.com/5uwifi/canchain/basis/swarm/api/http"
	"github.com/5uwifi/canchain/basis/swarm/fuse"
	"github.com/5uwifi/canchain/basis/swarm/log"
	"github.com/5uwifi/canchain/basis/swarm/network"
	"github.com/5uwifi/canchain/basis/swarm/network/stream"
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	if config.SwapAPI != "" && config.SwapEnabled {
		...
		backend, err = canclient.Dial(config.SwapAPI)
		if err != nil {
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config, privkey *ecdsa.PrivateKey) (*ensClient, error) {
	...
	ethClient := canclient.NewClient(client)
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := canclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func (self *Swarm) Start(srv *p2p.Server) error {
	...
	// update uaddr to correct ccnode
