...
func (s *WMailServer) Archive(env *whisper.Envelope) {
	...
	if err != nil {
		log4j.Error(fmt.Sprintf("rlp.EncodeToBytes failed: %s", err))
	} else {
		err = s.db.Put(key.raw, rawEnvelope, nil)
		if err != nil {
			log4j.Error(fmt.Sprintf("Writing to DB failed: %s", err))
		}
	}
}
...
func (s *WMailServer) DeliverMail(peer *whisper.Peer, request *whisper.Envelope) {
	if peer == nil {
		log4j.Error("Whisper peer is nil")
		return
	}
...
func (s *WMailServer) processRequest(peer *whisper.Peer, lower, upper uint32, bloom []byte) []*whisper.Envelope {
	...
	for i.Next() {
		...
		if err != nil {
			log4j.Error(fmt.Sprintf("RLP decoding failed: %s", err))
		}
		...
		if whisper.BloomFilterMatch(bloom, envelope.Bloom()) {
			if peer == nil {
				...
				if err != nil {
					log4j.Error(fmt.Sprintf("Failed to send direct message to peer: %s", err))
					return nil
				}
			}
		}
	...
	err = i.Error()
	if err != nil {
		log4j.Error(fmt.Sprintf("Level DB iterator error: %s", err))
	}
...
func (s *WMailServer) validateRequest(peerID []byte, request *whisper.Envelope) (bool, uint32, uint32, []byte) {
	...
	if decrypted == nil {
		log4j.Warn(fmt.Sprintf("Failed to decrypt p2p request"))
		return false, 0, 0, nil
	}
	...
	if src == nil {
		log4j.Warn(fmt.Sprintf("Wrong signature of p2p request"))
		return false, 0, 0, nil
	}
	...
	if payloadSize < 8 {
		log4j.Warn(fmt.Sprintf("Undersized p2p request"))
		return false, 0, 0, nil
	} else if payloadSize == 8 {
		bloom = whisper.MakeFullNodeBloom()
	} else if payloadSize < 8+whisper.BloomFilterSize {
		log4j.Warn(fmt.Sprintf("Undersized bloom filter in p2p request"))
		return false, 0, 0, nil
	} else {
