...
func (fs *Filters) NotifyWatchers(env *Envelope, p2pMessage bool) {
	...
	for _, watcher := range fs.watchers {
		...
		if p2pMessage && !watcher.AllowP2P {
			log4j.Trace(fmt.Sprintf("msg [%x], filter [%d]: p2p messages are not allowed", env.Hash(), i))
			continue
		}
		...
		if msg != nil {
			...
			if match {
				...
				if msg == nil {
					log4j.Trace("processing message: failed to open", "message", env.Hash().Hex(), "filter", i)
				}
			} else {
				log4j.Trace("processing message: does not match", "message", env.Hash().Hex(), "filter", i)
			}
		}
...
func (f *Filter) processEnvelope(env *Envelope) *ReceivedMessage {
	if f.MatchEnvelope(env) {
		...
		if msg != nil {
			...
			log4j.Trace("processing envelope: failed to open", "hash", env.Hash().Hex())
		}
	} else {
		log4j.Trace("processing envelope: does not match", "hash", env.Hash().Hex())
	}
	return nil
}
