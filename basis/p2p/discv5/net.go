...
loop:
	for {
		...
		select {
		case <-net.closeReq:
			log4j.Trace("<-net.closeReq")
			break loop

		// Ingress packet handling.
		case pkt := <-net.read:
			//fmt.Println("read", pkt.ev)
			log4j.Trace("<-net.read")
			...
			log4j.Trace("", "msg", log4j.Lazy{Fn: func() string {
		...
		case req := <-net.topicRegisterReq:
			...
			if topicRegisterLookupTarget.target == (common.Hash{}) {
				log4j.Trace("topicRegisterLookupTarget == null")
		...
		case nodes := <-topicRegisterLookupDone:
			log4j.Trace("<-topicRegisterLookupDone")
		...
		case <-topicRegisterLookupTick.C:
			log4j.Trace("<-topicRegisterLookupTick")
			if (topicRegisterLookupTarget.target == common.Hash{}) {
				target, delay := net.ticketStore.nextRegisterLookup()
				topicRegisterLookupTarget = target
				topicRegisterLookupTick.Reset(delay)
		...
		case req := <-net.topicSearchReq:
			if refreshDone == nil {
				log4j.Trace("<-net.topicSearchReq")
		...
		case <-refreshDone:
			log4j.Trace("<-net.refreshDone", "table size", net.tab.count)
			if net.tab.count != 0 {
				refreshDone = nil
	...
	log4j.Trace("loop stopped")

	log4j.Debug(fmt.Sprintf("shutting down"))
	if net.conn != nil {
		net.conn.Close()
	}
	if refreshDone != nil {
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	if len(seeds) == 0 {
		log4j.Trace("no seed nodes found")
		close(done)
		return
	}
	for _, n := range seeds {
		log4j.Debug("", "msg", log4j.Lazy{Fn: func() string {
			var age string
			if net.db != nil {
...
func (net *Network) ping(n *Node, addr *net.UDPAddr) {
	...
	log4j.Trace("Pinging remote node", "node", n.ID)
...
func (net *Network) handlePing(n *Node, pkt *ingressPacket) {
	log4j.Trace("Handling remote ping", "node", n.ID)
	ping := pkt.data.(*ping)
	n.TCP = ping.From.TCP
...
func (net *Network) handleKnownPong(n *Node, pkt *ingressPacket) error {
	log4j.Trace("Handling known pong", "node", n.ID)
	...
	if err == nil {
		...
		log4j.Trace("Failed to convert pong to ticket", "err", err)
	}
	n.pingEcho = nil
	n.pingTopics = nil
	return err
...
func (net *Network) handleNeighboursPacket(n *Node, pkt *ingressPacket) error {
	...
	for i, rn := range req.Nodes {
		...
		if err != nil {
			log4j.Debug(fmt.Sprintf("invalid neighbour (%v) from %x@%v: %v", rn.IP, n.ID[:8], pkt.remoteAddr, err))
			continue
		}
		nodes[i] = nn
