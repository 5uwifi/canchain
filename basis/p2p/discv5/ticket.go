...
func (s *ticketStore) addTopic(topic Topic, register bool) {
	log4j.Trace("Adding discovery topic", "topic", topic, "register", register)
...
func (s *ticketStore) removeRegisterTopic(topic Topic) {
	log4j.Trace("Removing discovery topic", "topic", topic)
	if s.tickets[topic] == nil {
		log4j.Warn("Removing non-existent discovery topic", "topic", topic)
		return
	}
...
func (s *ticketStore) nextRegisterLookup() (lookupInfo, time.Duration) {
	...
	for len(s.regQueue) > 0 {
		...
		if s.tickets[topic].nextLookup < mclock.Now() {
			...
			log4j.Trace("Found discovery topic to register", "topic", topic, "target", next.target, "delay", delay)
			return next, delay
		}
	}
	// No registration topics found or all exhausted, sleep
	delay := 40 * time.Second
	log4j.Trace("No topic found to register", "delay", delay)
...
func (s *ticketStore) ticketsInWindow(topic Topic) []ticketRef {
	...
	if s.tickets[topic] == nil {
		log4j.Warn("Listing non-existing discovery tickets", "topic", topic)
		return nil
	}
	...
	log4j.Trace("Retrieved discovery registration tickets", "topic", topic, "from", s.lastBucketFetched, "tickets", len(tickets))
	return tickets
}
...
func (s *ticketStore) addTicketRef(r ticketRef) {
	...
	if tickets == nil {
		log4j.Warn("Adding ticket to non-existent topic", "topic", topic)
		return
	}
...
func (s *ticketStore) nextFilteredTicket() (*ticketRef, time.Duration) {
	...
	for {
		...
		log4j.Trace("Found discovery ticket to register", "node", ticket.t.node, "serial", ticket.t.serial, "wait", wait)
...
func (s *ticketStore) removeTicketRef(ref ticketRef) {
	log4j.Trace("Removing discovery ticket reference", "node", ref.t.node.ID, "serial", ref.t.serial)
	...
	if tickets == nil {
		log4j.Trace("Removing tickets from unknown topic", "topic", topic)
		return
	}
...
func (s *ticketStore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
	log4j.Trace("Adding discovery ticket", "node", ticket.node.ID, "serial", ticket.serial)
...
func (s *ticketStore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
		log4j.Trace("Retrieving node ticket", "node", node.ID, "serial", nil)
	} else {
		log4j.Trace("Retrieving node ticket", "node", node.ID, "serial", s.nodes[node].serial)
	}
	return s.nodes[node]
}
