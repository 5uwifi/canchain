...
func ListenUDP(priv *ecdsa.PrivateKey, conn conn, realaddr *net.UDPAddr, nodeDBPath string, netrestrict *netutil.Netlist) (*Network, error) {
	...
	log4j.Info("UDP listener up", "net", net.tab.self)
	transport.net = net
...
func (t *udp) sendPacket(toid NodeID, toaddr *net.UDPAddr, ptype byte, req interface{}) (hash []byte, err error) {
	...
	log4j.Trace(fmt.Sprintf(">>> %v to %x@%v", nodeEvent(ptype), toid[:8], toaddr))
	if nbytes, err := t.conn.WriteToUDP(packet, toaddr); err != nil {
		log4j.Trace(fmt.Sprint("UDP send failed:", err))
	} else {
...
func encodePacket(priv *ecdsa.PrivateKey, ptype byte, req interface{}) (p, hash []byte, err error) {
	...
	if err := rlp.Encode(b, req); err != nil {
		log4j.Error(fmt.Sprint("error encoding packet:", err))
		return nil, nil, err
	}
	packet := b.Bytes()
	sig, err := crypto.Sign(crypto.Keccak256(packet[headSize:]), priv)
	if err != nil {
		log4j.Error(fmt.Sprint("could not sign packet:", err))
		return nil, nil, err
	}
...
func (t *udp) readLoop() {
	...
	for {
		...
		if netutil.IsTemporaryError(err) {
			...
			log4j.Debug(fmt.Sprintf("Temporary read error: %v", err))
			continue
		} else if err != nil {
			// Shut down the loop for permament errors.
			log4j.Debug(fmt.Sprintf("Read error: %v", err))
			return
		}
...
func (t *udp) handlePacket(from *net.UDPAddr, buf []byte) error {
	...
	if err := decodePacket(buf, &pkt); err != nil {
		log4j.Debug(fmt.Sprintf("Bad packet from %v: %v", from, err))
