...
func BenchmarkFilters(b *testing.B) {
	...
	var (
		db, _      = candb.NewLDBDatabase(dir, 0, 0)
	...
	genesis := kernel.GenesisBlockForTesting(db, addr1, big.NewInt(1000000))
	chain, receipts := kernel.GenerateChain(params.TestChainConfig, genesis, canhash.NewFaker(), db, 100010, func(i int, gen *kernel.BlockGen) {
		switch i {
		case 2403:
...
func TestFilters(t *testing.T) {
	...
	genesis := kernel.GenesisBlockForTesting(db, addr, big.NewInt(1000000))
	chain, receipts := kernel.GenerateChain(params.TestChainConfig, genesis, canhash.NewFaker(), db, 1000, func(i int, gen *kernel.BlockGen) {
		switch i {
		case 1:
			receipt := types.NewReceipt(nil, false, 0)
			receipt.Logs = []*types.Log{
