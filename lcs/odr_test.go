...
func testOdr(t *testing.T, protocol int, expFail uint64, fn odrTestFn) {
	...
	odr := NewLesOdr(ldb, light.NewChtIndexer(db, true), light.NewBloomTrieIndexer(db, true), can.NewBloomIndexer(db, light.BloomTrieFrequency), rm)
	...
	test := func(expFail uint64) {
		for i := uint64(0); i <= pm.blockchain.CurrentHeader().Number.Uint64(); i++ {
			...
			b1 := fn(light.NoOdr, db, pm.chainConfig, pm.blockchain.(*kernel.BlockChain), nil, bhash)
