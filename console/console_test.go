...
import (
	...
	"github.com/5uwifi/canchain/privacy/jsre"
)
...
type tester struct {
	workspace string
	stack     *node.Node
	ethereum  *can.CANChain
	console   *Console
	input     *hookedPrompter
	output    *bytes.Buffer
}

func newTester(t *testing.T, confOverride func(*can.Config)) *tester {
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return can.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register CANChain protocol: %v", err)
	}
