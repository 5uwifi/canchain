...
import (
	...
	"github.com/5uwifi/canchain/internal/jsre"
	"github.com/5uwifi/canchain/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jcan", struct{}{})

	jcanObj, _ := c.jsre.Get("jcan")
	jcanObj.Object().Set("send", bridge.Send)
	jcanObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jcan);"); err != nil {
	...
	flatten := "var can = web3.eth; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jcan.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jcan.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jcan.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jcan.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jcan.sign = personal.sign;`); err != nil {
