...
func (b *SimulatedBackend) CallContract(ctx context.Context, call canchain.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call canchain.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call canchain.CallMsg) (uint64, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call canchain.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, uint64, bool, error) {
	...
	return kernel.NewStateTransition(vmenv, msg, gaspool).TransitionDb()
}
...
type filterBackend struct {
	db candb.Database
	bc *kernel.BlockChain
}

func (fb *filterBackend) ChainDb() candb.Database  { return fb.db }
...
func (fb *filterBackend) SubscribeNewTxsEvent(ch chan<- kernel.NewTxsEvent) event.Subscription {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		<-quit
		return nil
	})
}
func (fb *filterBackend) SubscribeChainEvent(ch chan<- kernel.ChainEvent) event.Subscription {
	return fb.bc.SubscribeChainEvent(ch)
}
func (fb *filterBackend) SubscribeRemovedLogsEvent(ch chan<- kernel.RemovedLogsEvent) event.Subscription {
